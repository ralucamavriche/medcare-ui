...
const AppointmentPage = () => {
  ...
  const hasValidDoctorContract =
    requestedDoctorStatus === REQUEST_STATUSES.ACCEPTED && doctorId;
  const hasDoctorRequestPending =
    requestedDoctorStatus === REQUEST_STATUSES.SENT && doctorId;
  const hasNoRequest = requestedDoctorStatus === REQUEST_STATUSES.NOT_SENT;
      ...
      {hasValidDoctorContract && (
        <Container>
          ...
          <Typography color="text.secondary" variant="body2">
            Secure Your Time with Our Trusted Doctor for Personalized Care and Peace of Mind
          </Typography>
          <Calendar />
        </Container>
      )}
